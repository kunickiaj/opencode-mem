...
def test_api_usage_totals_respect_project_filter(tmp_path: Path, monkeypatch) -> None:
    ...
    try:
        ...
        conn.execute(
            "INSERT INTO sessions(id, started_at, project) VALUES (1, '2026-01-01T00:00:00Z', 'codemem')"
        )
        conn.execute(
    ...
    monkeypatch.setenv("CODEMEM_VIEWER_DEBUG", "1")
    ...
    try:
        ...
        filtered = _get_json(f"http://127.0.0.1:{port}/api/usage?project=codemem")
        assert filtered["project"] == "codemem"
...
def test_normalize_projects_rewrites_usage_event_project(tmp_path: Path) -> None:
    ...
    try:
        ...
        conn.execute(
            ...
            """,
            (json.dumps({"project": "/Users/adam/workspace/codemem"}),),
        )
        conn.commit()
    finally:
        conn.close()
