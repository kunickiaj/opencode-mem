...
DEFAULT_KEYS_DIR = Path("~/.config/codemem/keys").expanduser()
PRIVATE_KEY_NAME = "device.key"
PUBLIC_KEY_NAME = "device.key.pub"
KEYCHAIN_SERVICE = "codemem-sync"
...
def _warn_keychain_limitations() -> None:
    ...
    if _key_store_mode() != "keychain":
        return
    if os.environ.get("CODEMEM_SYNC_KEYCHAIN_WARN") == "0":
        return
    print(
...
def _load_device_id(db_path: Path | None = None) -> str | None:
    path = Path(db_path or os.environ.get("CODEMEM_DB") or db.DEFAULT_DB_PATH)
