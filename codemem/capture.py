...
TRUNCATION_NOTICE = "\n[codemem] transcript truncated\n"


def _max_transcript_bytes() -> int:
    raw = os.environ.get("CODEMEM_MAX_TRANSCRIPT_BYTES", "")
    if raw:
        try:
            value = int(raw)
        except ValueError:
            warnings.warn(
                "CODEMEM_MAX_TRANSCRIPT_BYTES must be an integer; using default.",
                RuntimeWarning,
                stacklevel=2,
            )
    ...
    if value < 0:
        warnings.warn(
            "CODEMEM_MAX_TRANSCRIPT_BYTES < 0 disables transcript capture.",
            RuntimeWarning,
            stacklevel=2,
        )
        value = 0
...
def run_command_with_capture(cmd: list[str], cwd: str | None = None) -> CommandResult:
    try:
        ...
        message = f"codemem: command not found: {cmd[0]}\n"
