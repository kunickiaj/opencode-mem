    ...
    *,
    ...
    if install:
        ...
        if sys.platform.startswith("darwin") and status.detail.startswith("failed (EX_CONFIG"):
            print(
                "[yellow]launchd cannot run codemem in dev mode; using pidfile daemon. Use `sync install` only after installing codemem on PATH.[/yellow]"
            )
        else:
...
def sync_pair_cmd(
    *,
    ...
    try:
        ...
        print("  codemem sync pair --accept-file pairing.json")
        print("If you prefer inline JSON, run:")
        print(f"  codemem sync pair --accept '{escaped}'")
        print("For machine-friendly output next time, run:")
        print("  codemem sync pair --payload-only")
...
def sync_doctor_cmd(
    *,
    ...
    try:
        ...
        if device is None:
            print("- Identity: missing (run `codemem sync enable`)")
        else:
...
def sync_daemon_cmd(
    *,
    ...
    config = load_config()
    if not config.sync_enabled:
        print("[yellow]Sync is disabled (enable via `codemem sync enable`).[/yellow]")
        raise typer.Exit(code=1)
    run_sync_daemon(
...
def sync_install_cmd(*, user: bool, system: bool) -> None:
    ...
    if sys.platform.startswith("darwin"):
        ...
        print("Run: launchctl load -w ~/Library/LaunchAgents/com.codemem.sync.plist")
        return
