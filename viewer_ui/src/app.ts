...
const detailsToggle = document.getElementById('detailsToggle') as HTMLButtonElement | null;
...
let lastStatsPayload: any = null;
let lastUsagePayload: any = null;
let lastRawEventsPayload: any = null;
...
function secondsSince(value: any): number | null {
...
function formatAgeShort(seconds: number | null) {
...
function parsePercentValue(label: any): number | null {
...
function buildHealthCard({
  ...
  if (title) {
    ...
    (card as any).style.cursor = 'help';
  }
  if (icon) {
    const iconNode = document.createElement('i');
    iconNode.setAttribute('data-lucide', String(icon));
    (iconNode as any).className = 'stat-icon';
...
async function copyCommand(command: string, button: HTMLButtonElement) {
...
function renderActionList(container: HTMLElement | null, actions: any[]) {
  if (!container) return;
  container.textContent = '';
  const list = Array.isArray(actions)
    ? actions.filter((item) => item && item.command)
    : [];
  if (!list.length) {
...
function renderHealthOverview() {
  ...
  const raw =
    lastRawEventsPayload && typeof lastRawEventsPayload === 'object'
      ? lastRawEventsPayload
      : {};
  ...
  const items = syncDisabled
    ...
    : [
        ...
        { label: 'Pending events', value: pending },
        {
          label: 'Last sync',
          value: lastSync
            ? `${formatAgeShort(secondsSince(lastSync))} ago`
            : 'never',
        },
        {
          label: 'Last ping',
          value: lastPing
            ? `${formatAgeShort(secondsSince(lastPing))} ago`
            : 'never',
        },
      ];
  items.forEach((item) => {
...
function renderPairing(payload: any) {
  ...
  if (pairingHint) {
    pairingHint.textContent =
      typeof payload.pairing_filter_hint === 'string' && payload.pairing_filter_hint.trim()
        ? payload.pairing_filter_hint
        : PAIRING_FILTER_HINT;
  }
}
